...
public abstract class TestObject {
    ...
    protected String toUriFragment(EObject document, String path) {
        ...
        while (i < parts.length) {
            ...
            String featureName = toFeatureName(segment);
            if (segment.endsWith("List")) {
                if (i + 1 >= parts.length || !parts[i + 1].matches("\\d+")) {
                    break;
                }
                int index = Integer.parseInt(parts[i + 1]) - 1;
                sb.append("/@").append(featureName).append(".").append(index);
                i += 2;
            } else {
    ...
    private String toFeatureName(String elementType) {
        return Character.toLowerCase(elementType.charAt(0)) + elementType.substring(1);
    }

    protected Object getChildNode(Object parent, String elementType, int index) {
        EObject eParent = (EObject) parent;
        String featureName = toFeatureName(elementType);
        EStructuralFeature feature = eParent.eClass().getEStructuralFeature(featureName);
        if (feature == null) {
            throw new IllegalArgumentException(
                    "No feature '" + featureName + "' on " + eParent.eClass().getName());
        }
